...
- **Decision — drop Phase 6 (the Chromium extension).** Its only unique value is reading arbitrary *live* pages in place, which conflicts with Tsumugu's batch/**pre-baked** design: a live page has no pre-generated explanations, so an overlay degrades to a Migaku-style popup dictionary — exactly what the browser packs already provide *inside* the web app. The "read this page" case is already served by Web Clipper → Inbox → `gen prep` → reader, which *adds* the pre-baked layer an extension can't. We keep at most a future trivial "clip selection → Inbox" button; we do not build the full overlay.
